...
# Tufte Layout Exerciser

<span class="newthought">This paragraph exists</span> to stress-test the Tufte layout elements we rely on (sidenotes, margin notes, full-width blocks, and code).

Here is an inline sidenote reference<label for="sn-demo-1" class="margin-toggle sidenote-number"></label><input type="checkbox" id="sn-demo-1" class="margin-toggle"/><span class="sidenote">A sidenote should float to the right on desktop and become toggleable on small screens.</span> and here is a margin note<label for="mn-demo-1" class="margin-toggle">⊕</label><input type="checkbox" id="mn-demo-1" class="margin-toggle"/><span class="marginnote">A margin note is similar, but without numbering. It’s good for small asides and definitions.</span>.

A full-width figure:

<figure class="fullwidth">
  <img src="/assets/img/ogp.png" alt="Demo image used for a full-width figure" />
  <figcaption>Full-width figure with a caption. (This is <code>assets/img/ogp.png</code>.)</figcaption>
</figure>

A full-width code block (Pandoc div syntax):
